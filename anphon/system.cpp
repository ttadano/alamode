...
void System::get_minimum_distances(KpointMeshUniform *kmesh_in,
                                   MinimumDistList ***&mindist_list_out)
{
    ...
    const int nkx = static_cast<int>(kmesh_in->nk_i[0]); // This should be int (must not be unsigned int).
    const int nky = static_cast<int>(kmesh_in->nk_i[1]); // same as above
    const int nkz = static_cast<int>(kmesh_in->nk_i[2]); // same as above
    ...
    for (i = 0; i < nat; ++i) {
        rotvec(xf_p[i], xr_s[map_p2s[i][0]], lavec_s);
        rotvec(xf_p[i], xf_p[i], rlavec_p);
    ...
    for (i = 0; i < ncell_s; ++i) {
        for (j = 0; j < ncell; ++j) {
            for (iat = 0; iat < nat; ++iat) {
                ...
                rotvec(x_all[i][j][iat], x_all[i][j][iat], lavec_p);
            }
        }
    }
