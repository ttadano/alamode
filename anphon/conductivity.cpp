                                           ...
                                           double **lifetime)
{
    ...
    if (kappa_intra) {
        memory->deallocate(kappa_intra);
        kappa_intra = nullptr;
    }
    memory->allocate(kappa_intra, ntemp, 3, 3);
    ...
    for (i = 0; i < ntemp; ++i) {
        for (unsigned int j = 0; j < 3; ++j) {
            for (unsigned int k = 0; k < 3; ++k) {
                ...
                kappa_coherent[i][j][k] *= factor_toSI * 1.0e+12 * time_ry / static_cast<double>(nk);
            }
        }
    }
}
