...
3. Prepare an input file for *alm*
  ...
  * ``PREFIX``.ANHARM?_pattern (If ``NORDER`` > 1)
  ...
  In pattern files, all necessarily displacement patterns are given in **Cartesian coordinates**.
...
4. Perform SCF calculations to generate displacement-force data set
  ...
  Then, please prepare necessarily input files for a DFT engine (or a classical force field engine) 
  ...
  .. Note::
    ...
    The script files can be found in the tools/ directory. We are planning to support other softwares. 
