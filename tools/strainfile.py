...
def main(argv=None):
    ...
    k.add_argument("--strain-ifc-dir", default=None, help="STRAIN_IFC_DIR with the text files and strained FC files")
    k.add_argument("--c1", default=None, help="C1_array.in (default: STRAIN_IFC_DIR/C1_array.in if present)")
    k.add_argument("--fcs", required=True, help="force-constant file (FC2FILE/FCSFILE) of the anphon run")
    k.add_argument("--anphon-cell", default=None, help="anphon input with the &cell field (required with an xml --fcs)")
    k.add_argument("--legacy-cell", default=None,
    ...
    k.add_argument("--force", action="store_true", help="overwrite an existing container")

    s = sub.add_parser("show", help="print the contents of a container")
    s.add_argument("file")
    s.add_argument("--min-c3", type=float, default=0.5, help="print TOEC components above this value (GPa)")

    c = sub.add_parser("check", help="check a container against a planned anphon run")
    c.add_argument("file")
    c.add_argument("--anphon-cell", default=None, help="anphon input with the &cell field")
    c.add_argument("--fcs", default=None, help="force-constant file (FC2FILE/FCSFILE) of the anphon run")

    args = p.parse_args(argv)
    try:
        if args.cmd == "pack":
            sf.pack(args.output, args.strain_ifc_dir, args.c1, args.fcs, args.anphon_cell, args.legacy_cell, args.force)
        elif args.cmd == "show":
